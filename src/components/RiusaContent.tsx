  ...
  return (
    ...
    >
      ...
      <span>
        ...
        <span className="mt-1 block text-sm text-[var(--pa-muted)]">{forWhat}</span>
      </span>
    </a>
  );
}
...
const OPTIONAL: { area: string; text: string }[] = [
  ...
  },
  {
    ...
    text: "Si crea con BotFather. Serve solo se vuoi che i cittadini segnalino nuovi DAE. Dettagli in docs/dae-telegram-bot.md nel template.",
  },
  {
...
export function RiusaContent() {
  ...
  return (
    <LandingDoc
      ...
      lede="Non serve essere programmatori. Con un account GitHub, uno su Vercel e circa un’ora puoi avere un sito con i dati aperti del tuo comune."
    >
      <p>
        Ogni comune ha il suo sito: non c’è un selettore con tutti i comuni
        d’Italia. I numeri arrivano da{" "}
        <a
      ...
      <div className="not-prose mb-2 flex flex-wrap gap-2">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[var(--pa-primary)] px-3 py-2 text-sm font-semibold text-white no-underline hover:bg-[var(--pa-primary-hover)]"
        >
          <VercelMark />
          Pubblica su Vercel
        </a>
      ...
      <p>
        Non ti servono carta di credito, server propri né un ufficio
        informatico. Se a un certo punto ti blocchi, scrivi a{" "}
        <a href={`mailto:${AUTHOR.email}`}>{AUTHOR.email}</a>.
      </p>

      <h2 className="guide-h2">Account da aprire (tutti gratis)</h2>
      <p>
        Per andare online ne bastano due. Gli altri arrivano dopo, se ti
        servono meteo extra, un bot o l’assistente.
      </p>
      <div className="not-prose account-grid mb-4">
        ...
        <AccountCard
          ...
          need="Facoltativo"
          forWhat="Meteo più ricco. Senza chiave il meteo di base resta."
        ...
        <AccountCard
          ...
          need="Facoltativo"
          forWhat="Solo se vuoi un bot per segnalare i defibrillatori."
      ...
      <ol className="not-prose step-list mb-6">
        ...
        <Step n={4} title="Scrivi il nome del tuo comune">
          <p className="m-0">
            ...
            <code>config/comune.json</code> (se non c’è, copia{" "}
            <code>config/comune.example.json</code>). Premi la matita in alto a
            destra. Cambia almeno:
          </p>
          <ul className="mb-0 mt-2 list-disc pl-5">
            ...
            <li>
              ...
              abitato (OpenStreetMap: tasto destro → «Mostra indirizzo»)
            </li>
            <li>
              <code>fork.is_template</code> → <code>false</code>,{" "}
              <code>site.mode</code> → <code>dashboard</code>, e i tuoi nome e
              e-mail in <code>fork.maintainer_*</code>
            </li>
          </ul>
        ...
        <Step n={6} title="(Facoltativo) Il tuo dominio">
          <p className="m-0">
            ...
            <code>brand.site_url</code>. Senza dominio resta l’indirizzo Vercel,
            ed è già un sito vero.
          </p>
        </Step>
      </ol>
      ...
      <p>
        Il file{" "}
        <a
          href={`${github}/blob/main/config/comune.example.json`}
          target="_blank"
          rel="noopener noreferrer"
        >
      ...
      <div className="not-prose overflow-x-auto rounded-lg border border-[var(--pa-border)] bg-white">
        <table className="w-full min-w-[28rem] border-collapse text-left text-sm">
          <caption className="sr-only">Campi del file comunale da compilare</caption>
          <thead>
          ...
          <tbody>
            {[
              [
                "istat_code",
                "Chiave dei dati AgID. Senza questo, i numeri restano di un altro comune.",
              ],
              [
                "nome, provincia, regione",
                "Testi in testata, piè di pagina e motori di ricerca.",
              ],
              ["geo.map_center e geo.meteo", "Punto da cui partono mappa e previsioni."],
              [
                "geo.terrain_sea_side",
                "Lato mare del rilievo 3D. Se non hai il mare, scrivi none.",
              ],
              ["miur_codice_catastale", "Scuole. Quattro caratteri, es. I390."],
              ["farmacie_di_turno_cod", "ISTAT senza lo zero iniziale."],
              [
                "features.*",
                "Interruttori: false nasconde porto, spiagge, treni, bot…",
              ],
              [
                "fork.*",
                "Chi mantiene QUESTO sito. is_template: false nei fork.",
              ],
              [
                "sostieni.buymeacoffee_slug",
                "Pagina Supporto. Vuoto = la pagina sparisce. Non copiare lo slug dell’autore.",
              ],
            ].map(([campo, dove]) => (
              <tr key={campo} className="border-b border-[var(--pa-border)] last:border-0">
                ...
                <td className="px-3 py-2.5 align-top text-[var(--pa-muted)]">{dove}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p className="mt-4">
        ...
        <a
          href={`${github}/blob/main/.env.example`}
          target="_blank"
          rel="noopener noreferrer"
        >
          .env.example
      ...
      <p>
        In locale: Node 20+, <code>npm install && npm run dev</code>, poi apri
        localhost:3000. Dopo aver cambiato ISTAT e coordinate:
      </p>
...
npm run trasporti:gtfs`}
      ...
      <p>
        ...
        Tieni aperto <code>AGENTS.md</code>. Non serve costruire un server dati:
        i KPI arrivano già da AgID. Guida nel template:{" "}
        <a
          href={`${github}/blob/main/docs/riuso-fork.md`}
          target="_blank"
          rel="noopener noreferrer"
        >
          docs/riuso-fork.md
      ...
      <ul className="list-disc space-y-1.5 pl-5">
        ...
        <li>Lo stemma e il nome in testata sono i tuoi.</li>
        <li>Porto, spiagge e bot sono spenti se non ti servono.</li>
        <li>La frase «progetto non ufficiale» è ancora lì. Va tenuta.</li>
        <li>Nessuna chiave segreta è finita nel codice.</li>
      </ul>
      <p>
        Per l’amministrazione: testi pronti nel{" "}
        <Link href="/kit-ente">kit ente</Link>
        {" · "}
